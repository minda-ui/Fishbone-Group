...
Ten other knowledge systems exist (the Fishbone Holdings Ltd Knowledge Base was added 2026-09-05, the
...
| Group Tasks tracker | Smartsheet "1. General", **Tasks** sheet `1343219457722244` (per-company Tasks sheets share the same shape: Construction `5235584035587972`, Holdings `3298244547446660`, SSAS `8617681802626948`, AMFA `6815307366795140`) | Shared task list (`T#####`). Status picklist = **Open / In Progress / Done / Blocked**. **Status colour convention (2026-09-11, owner) — set via Smartsheet _conditional formatting_ in the UI, not the API: Done = green, In Progress = yellow, Overdue = red, where "Overdue" is a rule (Due Date in the past AND Status ≠ Done), not a Status value.** A future session must not try to add this via the connector — the Smartsheet API/MCP exposes no conditional-formatting tool. Read-only for automation (§6a). |
