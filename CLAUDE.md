...
> repo.
> Revised 2026-09-13: recorded a group-wide Drive-write lesson in §3d — a sister session (Eugene/email-
> architecture work) hit repeated file corruption using `base64Content` on Google Drive `create_file` for
> plain Markdown; the fix (confirmed against the tool's own documented guidance) is `textContent` for any
> UTF-8 text file, reserving `base64Content` for genuinely binary content. No owner input needed; a tooling
> correctness fix.
> Revised 2026-09-14: synced this git mirror against the live Drive database, which had drifted well ahead
> of it. Added **Eugene — AI IT Assistant** (the group's second AI employee, built 2026-09-12 night) to
> §0/§1; sister-systems count to **twelve**. Recorded the locked **AI Workforce Plan v2**
> (`Outputs/2026-09-12_Plan_AI-Workforce_v2.md` — a roster of 8 AI employees, one per function, build order
> Peter → Eugene → Content & Marketing → …) and the **built AI Workforce Operations Hub**
> (`Outputs/2026-09-14_Plan_AI-Workforce-Operations-Hub_v1.md` — Smartsheet workspace "Fishbone AI
> Workforce" + a private interactive Artifact board tracking roster/tasks/achievements; its daily reconcile
> routine is drafted but not yet created, §5). Updated Peter's row: 2 routines now live, and its group-inbox
> routing to the `ops@fishboneconstruction.co.uk` hub is complete for all six trading/holding entities
> (SSAS excluded by design), as of 2026-09-14. §0, §1 and §5 updated. No owner input needed; this git mirror
> was simply behind Drive, the source of truth.
...
**AI Workforce.** The group is standing up a roster of AI "employees" one at a time, under the
locked `Outputs/2026-09-12_Plan_AI-Workforce_v2.md`: **Peter** (data collection) and **Eugene** (IT
& engineering) are live — #1 and #2 — with Content & Marketing next. Each has its own KB (§1
sister systems) and inherits this file's §6a governance boundary — no employee acts outward or
writes to a system of record without a human, and Eugene additionally never holds secrets. The
shared **AI Workforce Operations Hub** (§1; a Smartsheet workspace + a private interactive
Artifact board) tracks their roster, tasks and achievements (`Outputs/2026-09-14_Plan_AI-Workforce-
Operations-Hub_v1.md`, BUILT).
...
and **Peter, the group's data-collection assistant, was added 2026-09-12 (evening), followed by
Eugene, the group's IT & engineering assistant, on 2026-09-12 (night)** — Peter and Eugene are the
first two of a planned roster of AI "employees"; see the AI Workforce Plan pointer in §0).
...
| Peter - AI Data Assistant | Drive root, `1zY8rVKNXheb8MQaol6GZthht1B7hlkAZ`, git mirror `minda-ui/Peter` | The group's **data-collection assistant** (own charter, four control files, and `Inbox-Triage`/`Research`/`Capture`/`_unverified` staging folders): reads the scoped **`ops@fishboneconstruction.co.uk`** inbox — now the single routing hub for all six trading/holding entities' mail (Construction, Properties, Commercial, Amfa, Holdings, Waste; SSAS excluded by design, 2026-09-14) — watches Companies House for the six registered companies, and stages qualifying documents toward the group Document Register. **Collects and stages only — reads and drafts email (`create_draft`), never sends, files with anyone, or writes to a system of record (§6a).** Hands findings to the company KBs / group register; cite, never copy. | Live (2026-09-12); **2 routines live** (daily inbox triage+capture, weekly Companies House research); group-inbox routing to the hub complete for all six entities 2026-09-14 |
| Eugene - AI IT Assistant | Drive root, `1o4MBRcckZBspw-uT6qM2V-74H6OsRK9T`, git mirror `minda-ui/Eugene` | The group's **IT & engineering assistant** (own charter, four control files, `Runbooks`/`Infra-Inventory`/`Hardware-Projects` working folders): produces setup runbooks + config and verifies them, scaffolds new AI employees and drafts their routine prompts, writes/tests hardware & automation code, keeps the infra inventory — including the runbooks behind Peter's Google Workspace consolidation and inbox-routing rollout. **Edits code/repos/KB directly; guide-only for live systems** (Admin console/DNS/mailbox migration/account provisioning/hardware deploy — a human executes); **never holds secrets** (§6a). Connectors Drive + GitHub + Web (no Gmail); **interactive, no routines**. Cite, never copy. | New (2026-09-12 night); actively directing the AI-workforce infra rollout |
| AI Workforce Hub | Smartsheet workspace "Fishbone AI Workforce" (id `4946803578693507`, owner-only minda@) + a private interactive Artifact board (`https://claude.ai/code/artifact/bfe4bbc6-2718-409a-8512-2dc44988406b`) | The shared operational surface for the AI workforce (§0): **Roster** (who works here — seeded with Peter + Eugene), **Tasks & Requests** (assign work / ask a question; carries the group RYGB `Health` column formula), **Achievements** (completed-task rollup). Bridged by a daily reconcile between the board and the Smartsheet system of record (drafted, not yet a live routine — §5). See `Outputs/2026-09-14_Plan_AI-Workforce-Operations-Hub_v1.md` (BUILT) and the parent `Outputs/2026-09-12_Plan_AI-Workforce_v2.md` (the roster + build-order plan). Cite, never copy. | Built 2026-09-14; owner-only access |
...
### 3d. Reading and writing limits, inherited from the sister systems
- **Google Drive file writes (`create_file`/equivalent): use `textContent`, not `base64Content`,
  for any UTF-8 text file** (change-log entries, Wiki articles, owner notes, `CLAUDE.md` itself —
  everything this KB ecosystem writes is plain Markdown). The tool's own guidance reserves
  `base64Content` for genuinely non-UTF-8/binary content; using it for text adds an unneeded
  encode/decode round-trip and was observed causing write corruption and slow writes in a sister
  session's Eugene/email-architecture work (2026-09-13), fixed by switching to `textContent`. Never
  use the deprecated `content` field (always-base64 regardless of mime type) — it's exactly what
  causes this if reused out of habit. Treat `textContent` as the default for every text file;
  `base64Content` only for images, PDFs, or other binary payloads.
...
| AI Workforce Hub — daily reconcile | Daily, ~07:00 UK (proposed) | Reads the AI Workforce Hub board's `tasks` collection and the "Fishbone AI Workforce" Smartsheet Tasks/Achievements/Roster sheets (§1) and keeps them in step: promotes board-added requests into Smartsheet under new `AWT-####` ids, mirrors Status/Response both ways. Writes only to that Smartsheet workspace and the board `db` — nothing else, no sister KB touched. Connectors: Smartsheet + the Artifact tool (`read_db`/`write_db`) against the board URL. | **Proposed — drafted 2026-09-14** (`Outputs/2026-09-14_AI-Workforce-Hub-Reconcile-Routine-and-Grants_v1.md`); create via the routines form. Also needs a narrow charter grant applied in Peter's and Eugene's own KBs (§6a) so each may write Status/Response back to its own Hub rows. |
